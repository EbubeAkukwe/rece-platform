...
import { LocationHeading } from '@/features/listings/components/listings-with-location'
...
async function ListingsGrid({ searchParams }: { searchParams: SearchParams }) {
  ...
  let query = supabase
    .from('properties')
    .select('*, property_images(url, is_cover)', { count: 'exact' })
  ...
  if (searchParams.q) {
    query = query.or(
      `title.ilike.%${searchParams.q}%,city.ilike.%${searchParams.q}%,country.ilike.%${searchParams.q}%,address.ilike.%${searchParams.q}%`
    )
  } else {
    // Location-based filtering (passed as URL params from client)
